...
namespace game_of_life {
	...
	void Field::readStartFile() {
		...
		if (file.is_open()) {
			file >> rows >> columns;
			//установка размера вспомогательного вектора, который будет хранить предыдущее состо€ние в игре
			prevIsAlive.resize(rows);
			for (int i = 0; i < rows; ++i) {
				prevIsAlive[i].resize(columns);
			}
			//установка размера основного вектора и его инициализаци€ значени€ми из файла
	...
	void Field::updateFields() {
		prevIsAlive = vIsAlive; //сохран€ем предыдущее состо€ние
		++generation; //увеличиваем поколение

		// алгоритм изменени€ состо€ни€ €чейки  и алгоритм прохода по €чейкам пол€
		for (int i = 0; i < prevIsAlive.size(); ++i) {
			for (int j = 0; j < prevIsAlive[i].size(); ++j) {
				...
				if (prevIsAlive[i][j] == false) {
					...
					if (count == 3) { vIsAlive[i][j] = true; } //оживление мЄртвой клетки, если есть 3 живых соседа
				}
				//если клетка жива€
				else {
					int count{ 0 };
					...
					if (count == 3||count==2) { vIsAlive[i][j] = true; } //2 или 3 живых соседа - клетка живЄт
					else { vIsAlive[i][j] = false; } //в ином случае - умирает от одиночества или перенаселЄнности
				}
			}
		}
	...
	int Field::countingAliveCells(char mode) {
		//b = базовый полный подсчЄт(base), l = лЄгкий подсчЄт (light), возвращающий 0 - false или 1 - true
		if (mode == 'b') {
			int count{ 0 };
			for (int i = 0; i < vIsAlive.size(); ++i) {
				for (int j = 0; j < vIsAlive[i].size(); ++j) {
